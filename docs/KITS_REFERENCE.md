...
| Shot island | `<rv-shot kind="app\|dashboard" slug="{{ row.ref }}" title="…"></rv-shot>` | Artifact THUMBNAIL (the Hub, docs/HUB_PLAN.md). Handle-based — kind + slug, never a URL; the server resolves src to the lens thumb proxy (`/api/rvbbit/thumb`), which fetches from warehouse-mcp `/thumbs` with the static key. Missing capture degrades to a monogram tile. Queryless, and ALLOWED inside `rv-each` (unlike query-bound islands). |
