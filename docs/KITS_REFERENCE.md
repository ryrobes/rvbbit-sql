...
| `rvbbit.plates.surface` | `'ui'` (default) or `'logic'` (0204). LOGIC PLATES are headless: every declared query is a CHECK (rows = violations, empty = green) and the template is the human-written EXPLANATION agents read. Same authoring surface (upsert_plate/revisions/assistant); set via `UPDATE rvbbit.plates SET surface='logic'` (like module). Hidden from the launcher shelf; observable via kit_pulse |
| `rvbbit.kit_pulse(kit)` | The agentic rendering of the ruleset: one call → every check (kit contracts + logic-plate queries) with explanation, violation count, 3-row sample, green/red/error status. Red rows become SENTENCES in chat ("still need the backplate photo") and red modules on plates — same rows, two renderings. Param-bound (`{{ }}`) checks are skipped (they need a caller) |
| `rvbbit.kit_graph(kit)` | The app flow DERIVED from declared metadata — no new declarations: layout→contains→plate, plate→opens→plate (rv-open), plate→emits/param→drives (bus), plate→reads/checks→table, action→writes→table, contract→gates→module. `kit_graph_crawl(kit)` mirrors it into KG graph `kit_flow_<kit>` for the graph explorers |
| `rvbbit.kit_brief(kit)` | THE SKILL CARD (0206): the kit compiled into narrative markdown — description, nouns (tables + COMMENTs), the write API as signatures (`?` = optional arg), logic-plate explanations VERBATIM, every check's LIVE status with samples, writes, human surfaces, timestamped footer. Deterministic (no model), compiled at read time, and THE SAME document agents receive (`public.kit_brief` over run_sql) — the human reading the Kit Flow Briefing tab reads exactly what the agent reads. Confidence by construction |
| `kit_rehearsal` (warehouse-mcp tool) | The LINT on top (never the source): feeds the brief + a scenario to a model, returns the numbered runstream it would follow AND every ambiguity/missing verb it had to guess around. Each gap finding is either an edit to a logic plate's explanation or an action the kit still needs — a QA loop for workflows-as-prose. Timestamped, one model's traversal, never authoritative |
| `rvbbit.plate_layouts` | The compose layer (0187, docs/PLATE_COMPOSE_PLAN.md): kit-shipped free-floating compositions of plates. `layout_id` PK, `kit`, `design` jsonb (`{width,height,min_width}`), `panes` jsonb — each `{id, plate, x,y,w,h (fractions of design), z, params (pinned defaults), slot, title}`. `rvbbit.upsert_layout(...)` validates fractions/unique ids and REJECTS non-arrangement keys (the HyperCard wall: layouts own arrangement, never behavior). `kits.default_layout` = the kit's front door. Revisions mirror 0182 (`plate_layout_revisions` + `restore_layout`). Lens: wall mode (chromeless full-screen, ESC back, zoom-a-pane, wall-local modals for plain `rv-open="plate:x"`, slot targeting via `rv-open="plate:x@pane"`), stamp mode (panes → desktop windows), and save-arrangement (desktop windows → layout row — the desktop IS the editor). Assistant: `upsert_layout` (+`"default":true`) / `patch_layout` (panes merge per id, null removes) / `open_layout` |
| `rvbbit.plate_revisions` | Safety net (0182): a `BEFORE UPDATE OR DELETE` trigger snapshots the outgoing row (`snapshot` jsonb = whole row) on every content change; idempotent rewrites captured nothing; last 20 kept per plate. `rvbbit.restore_plate(plate_id, rev DEFAULT NULL)` re-materializes a revision (delete-then-insert, so the replaced state is itself ledgered — restores are undoable). The plate window's source menu lists revisions with built-not-run restore statements. Render instrumentation rides every render: the strip shows total ms (hover = per-query breakdown) and the source menu shows `ms · rows` (or `error`) beside each query |
...
| Grid editing | `<rv-grid query="q" edit-action="name" id="key_col" edit="qty,notes"></rv-grid>` | Spreadsheet cells (0185): double-click edits, Enter commits, Esc/blur cancels. Commit fires the action with `{id, column, value}` (declare exactly those three text args). `edit=` lists editable columns; omit it and every column except `id` is editable. Action idiom: ONE UPDATE with a CASE per column — `SET qty = CASE WHEN {{column}}='qty' THEN nullif({{value}},'')::int ELSE qty END … WHERE key = {{id}}`; never interpolate `{{column}}` as an identifier. The pending cell pulses; a failed action reverts. |
...
| Shot island | `<rv-shot kind="app\|dashboard" slug="{{ row.ref }}" title="…"></rv-shot>` | Artifact THUMBNAIL (the Hub, docs/HUB_PLAN.md). Handle-based — kind + slug, never a URL; the server resolves src to the lens thumb proxy (`/api/rvbbit/thumb`), which fetches from warehouse-mcp `/thumbs` with the static key. Thumbnails are SELF-HEALING: /thumbs enqueues a throttled capture on miss or staleness, and the island retries a couple of times, so a monogram upgrades to the real shot without user action. Content is pointer-events-none (portals bypass plate delegation; a thumbnail must never eat its card's click). Queryless, and ALLOWED inside `rv-each`. |
| Grid over dynamic SQL | `<rv-grid sql-from="query.column" limit="50"></rv-grid>` | A grid whose SQL CAME FROM SQL: the named query's first row holds the statement (how the Hub peek shows a cube's rows when the table name is only known per-selection). SELECT-shaped only, read-only execution, hard cap 200 rows; timed into the render debug as `grid:<ref>`. Failures render an inline error; an empty source renders nothing. |
| Frame island | `<rv-frame kind="app\|dashboard" slug="{{ row.ref }}" height="470"></rv-frame>` | LIVE artifact iframe (the Hub peek). Same handle-only contract; src resolves server-side to the warehouse origin (`RVBBIT_APP_BASE`) — `/apps/<slug>` or `/d/<slug>`. Sandboxed; the embedded app talks to ITS origin (rvbbitQuery), never lens. Viewer auth rides the browser's warehouse session (same registrable domain → cookies flow). Queryless, allowed inside `rv-each`. |
...
| Card gesture | `rv-emit` + `rv-open` on ONE `<button>` | The board's edit-loop gesture, button edition: emit publishes FIRST (no toggle), then rv-open fires — so `rv-open="plate:hub/peek@peek"` renders the target plate already scoped to the emitted param. How Hub gallery cards drive the peek slot. |
| Double-click open | `rv-open-dbl="app:live?slug=…&kind=…"` (alongside the card gesture) | The escalation gesture: double-click opens the artifact FOR REAL (apps/dashboards → their standalone desktop window; from a wall, the wall closes). On dbl-carrying elements the single-click pair is delayed one beat so the pair can be told apart — the emit's refetch unmounts DOM, so the lens matches the pair by attribute VALUE, never native dblclick. Note: rv-shot content is pointer-events-none (portal events don't reach plate delegation; a thumbnail must never eat its card's click). |
