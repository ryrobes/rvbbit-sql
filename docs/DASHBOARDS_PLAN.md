...
> works without Claude — and every significant number is a doorway into the
> warehouse, because its versioned semantic map preserves meaning, UI context, and
> independently replayable SQL without turning it into a governed metric.*
...
1. **The artifact version is the single source of truth.** It carries immutable HTML and an
   optional authored semantic map; an immutable, version-keyed compiler overlay fills its
   uncovered visible business values. We do *not* require a second set of governed
   metrics/cubes to keep in sync, and generated enrichment never rewrites authored source.
...
```
  Cowork: Claude builds + publishes ──► rvbbit.dashboards (immutable versioned HTML)
                                          │                 │
                              immediate live URL            └─► durable enrichment queue
                                                                  │
                                      headless render + DOM/query evidence
                                                                  │
                                      RVBBIT agent candidates + deterministic SQL replay
                                                                  │
                                      immutable semantic overlay ──► deps/edges
                                          │
...
For enriched artifacts, the Lens starts from named semantic values: plain-language meaning,
the exact DOM binding, current filter context, and an independently executable scalar SQL
definition. It recreates the selected value and keeps SQL collapsed as technical evidence.
One parameterized object can bind many chart labels or table cells with per-element context.

For older artifacts, we still know each panel's base query (extracted at publish, confirmed at runtime). In lens, a
...
rvbbit.dashboard_versions(dashboard_id, version, html text,   -- the artifact source
                          ...
                          manifest jsonb,          -- includes semantic_map v1
                          pinned_as_of timestamptz,           -- snapshot for board mode
                          created_by, created_at, notes)

-- DERIVED semantic compiler output, immutable-version keyed; authored source is untouched
rvbbit.artifact_semantic_enrichments
                         (dashboard_id, version, status, input_hash,
                          semantic_map jsonb, verification jsonb, agent_run_id,
                          model, prompt_version, attempts, last_error,
...
rvbbit.dashboard_deps    (dashboard_id, version, panel_hint,
                          kind text,              -- 'query' | 'semantic' | 'metric' | 'table'
...
- **`publish_dashboard(name, html, team?, description?, kind='live', manifest?)`** → validate
  any authored semantic evaluators (quarantine a bad optional map rather than reject the
  HTML), store the version, enqueue the non-blocking semantic compiler, run
  `dashboard_crawl`, and return the live URL immediately.
- **`list_dashboards(team?, search?)`** → semantic discover/reuse.
- **`get_dashboard(slug, version?)`** → source + deps (to fork or inspect).
- **`update_dashboard(slug, html, notes?, manifest?)`** → new version + re-extract; omitted
  manifest preserves the previous semantic map.
- **`semantic_enrichment_status(slug, version?)`** → queue state, verified/rejected counts,
  and semantic coverage for an immutable version.
- **`enrich_live_app(slug, version?, force?)`** → queue/requeue a version for backfill or an
  explicit compiler refresh; it never changes the artifact version itself.
