//! Duck sidecar/broker telemetry schema and exception-path writebacks.
//!
//! The hot-path sidecar telemetry is emitted by the `rvbbit-duck` process
//! itself. This module owns the SQL-facing schema and records rare Postgres
//! extension-side events, such as shared broker fallback.
...
pub(crate) fn record_shared_fallback(
    ...
    let node_id = std::env::var("RVBBIT_NODE_ID")
        .or_else(|_| std::env::var("RVBBIT_DUCK_NODE_ID"))
        .unwrap_or_else(|_| hostname.clone());
    let database_name = current_database_name().unwrap_or_else(|| "unknown".to_string());
    let role_name = current_user_name().unwrap_or_else(|| "unknown".to_string());
    let query_hash = sql_hash(query);
    let sql = format!(
        "INSERT INTO rvbbit.duck_sidecar_fallback_events \
         (hostname, node_id, backend_pid, database_name, role_name, engine, layout, socket_path, reason, fallback_mode, query_hash) \
         VALUES ({}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {})",
        ...
        quote_literal(layout),
        quote_nullable(socket_path),
        quote_literal(&reason.chars().take(2048).collect::<String>()),
        quote_literal(fallback_mode),
        quote_literal(&query_hash),
    );
    let _ = Spi::run(&sql);
}
...
fn quote_nullable(value: Option<&str>) -> String {
    match value {
