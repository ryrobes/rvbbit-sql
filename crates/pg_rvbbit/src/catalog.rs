...
-- rvbbit.compact(rel [, keep_heap]):
--   1. Writes the current heap contents of `rel` as a new immutable
--      parquet row group (with JSON shredding for known paths).
--   2. By default, TRUNCATEs the source heap — data is now ONLY in parquet.
--      With keep_heap=true, leaves the heap in place as an experimental
--      source-of-truth/shadow-rowstore probe. This is deliberately opt-in:
--      repeated keep_heap compactions can duplicate the same heap rows into
--      parquet until generation tracking lands.
