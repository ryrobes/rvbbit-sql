...
-- Per-table autovacuum tuning for the extension's own catalogs (0138).
-- Two tiers. 'volume' tables scale with user data (tombstones, identity
-- maps, staging rows) or append-heavy telemetry: Postgres' default
-- autovacuum_vacuum_scale_factor (0.2) means a 250M-row delete_log needs
-- ~50M dead tuples before autovacuum even looks at it — a small scale
-- factor plus a fixed threshold keeps dead-tuple debt bounded regardless of
-- size. 'hot' tables are tiny but update-churned catalogs where the real
-- bloat lives in TOAST (stats/jsonb payloads), which needs its own override.
--
...
-- stomped and re-running from the maintenance heartbeat is a near-free
-- no-op. Runs from rvbbit.maintain_storage so tables that bootstrap lazily
-- at runtime (route_decisions, fleet_endpoints, alert_*) converge too.
...
-- Orphaned-tombstone pruning (0138). Tombstones are LIVE data: every scan
-- projects them into skip-bitmaps, and rebuild_acceleration() is the only
-- path that retires them wholesale. But two paths strand them permanently:
-- (a) generation reaping deletes row_groups rows without touching
-- delete_log, and (b) tables dropped without the event trigger firing. A
-- tombstone whose (table_oid, rg_id) no longer exists in row_groups can
-- never influence any scan — latest or AS OF — because scans enumerate
-- row_groups first.
--
...
-- Tombstone-pressure visibility (0138). A table whose workload is "delete
-- everything, re-insert everything" (full-replace ELT) accumulates
-- tombstones equal to its row count every cycle — high tombstone_pct is the
-- signal that rvbbit.rebuild_acceleration(t) is due (it folds tombstones
-- into a fresh baseline and empties delete_log for the table). Counts scan
-- delete_log, so this is a diagnosis-time view, not a dashboard poller.
...
-- pg_cron VACUUM template (0138). Tuned reloptions fix the trigger math, but
-- an instance whose autovacuum workers are saturated by thousands of user
-- tables still benefits from a scheduled top-level VACUUM of the extension's
-- own catalogs. VACUUM cannot run inside a function — but pg_cron executes
-- job commands as top-level statements, so the JOB is the VACUUM itself;
-- this function only registers it. The table list is resolved at call time:
-- re-run after enabling features that add tables. Mirrors
-- rvbbit.schedule_accel_tick's cron-home handling.
...
-- Maintenance mode (0138). Formalizes the field procedure: flip
-- rvbbit.force_heap_scan at the database level so every NEW session routes
-- straight to the heap (correct answers, no acceleration) while you vacuum /
-- rebuild / debug, then flip it back. Existing sessions keep their current
-- behavior until they reconnect — pooled connections cycle through quickly
-- in practice.
...
BEGIN
    -- Mirrors register_backend's security posture: changing `embed` changes
    -- outbound calls made by many SQL helpers.
