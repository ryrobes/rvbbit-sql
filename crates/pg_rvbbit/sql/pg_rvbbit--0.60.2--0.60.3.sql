-- pg_rvbbit 0.60.2 -> 0.60.3
-- DuckDB/Vortex first-class routing candidate.

CREATE OR REPLACE FUNCTION rvbbit.duck_vortex_query_json(
    "query" text,
    "column_names" jsonb,
    "max_rows" integer
) RETURNS jsonb
STRICT VOLATILE
LANGUAGE c
AS 'MODULE_PATHNAME', 'duck_vortex_query_json_wrapper';
...
CREATE VIEW rvbbit.route_shape_summary AS
...
CREATE VIEW rvbbit.route_profile_summary AS
SELECT
