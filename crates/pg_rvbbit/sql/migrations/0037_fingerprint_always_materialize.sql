...
--
-- 0035 materialized a temp sample ONLY for tables larger than sample_rows; small
-- tables were profiled by pointing every per-column stat at the base relation
-- (v_src := rel). That is fine for a row-store heap, but PATHOLOGICAL for an
-- rvbbit-AM (columnar / accelerated) table: each aggregate against it carries
-- seconds of query-engine overhead AND serializes on a shared engine resource, so
-- ~31 columns × ~5 stats = 150+ aggregates took minutes for even a 300-row table
-- (e.g. application_data_public.doctors: a single `min(updated_at)` over 293 rows
-- ran >40s, while copying those 293 rows into a heap temp table = 27ms and all five
-- aggregates over the copy = 20ms).
--
-- Fix: ALWAYS materialize the working set into a plain row-store TEMP table first
-- (full copy when the table is small, TABLESAMPLE/LIMIT when it exceeds sample_rows),
-- so the per-column passes never touch a columnar relation directly — one scan, then
-- every stat hits a tiny local heap. Output + semantics unchanged (small tables are
-- still profiled exactly: v_sampled stays false, ndv_method 'exact'). Only the
-- materialization block differs from 0036.
...
BEGIN
    ...
    -- Row count via the planner's estimate, NOT a full count(*) scan (0036): on
    -- large rvbbit-AM / heap-alias tables count(*) is a multi-second full scan.
    -- reltuples is free + accurate for analyzed tables; exact count only when the
    -- estimate is missing/zero (never-analyzed or truly empty).
    ...
    -- ALWAYS materialize the working set into a plain row-store TEMP table, then run
    -- every per-column stat below against THAT (never the base relation). Critical
    -- for rvbbit-AM (columnar) tables: a direct aggregate carries seconds of engine
    -- overhead + serializes on a shared resource, so 150+ per-column aggregates on a
    -- small accelerated table took minutes. One scan into a heap temp collapses that;
    ...
    ELSE
        ...
        -- table (still avoids per-column aggregates hitting the columnar store).
