-- 0159: system plates ship with the product (KIT_PLATES_PLAN dogfood → default).
--
-- Plates are DATA — this migration is pure upsert_plate() calls, idempotent
-- by construction. system/health is the maintenance surface (health cards,
-- tombstone top-10, built-not-run remedies, cron state; the lens routes the
-- cron.* queries to pg_cron's home db). rvbbit/welcome is the product's own
-- switchboard: readiness cards + starter scripts for a fresh install.
...
-- ── rvbbit/welcome: the product's own switchboard ──────────────────────
SELECT rvbbit.upsert_plate(
  'rvbbit/welcome',
  'Welcome to rvbbit',
  $tpl$
<div class="plate-section">
  ...
  <div class="plate-cards">
    <div rv-each="steps" class="plate-card {{ row.tone }}">
      <div class="plate-card-title">{{ row.name }}</div>
      <div class="plate-card-value">{{ row.value }}</div>
      <div class="plate-card-note">{{ row.note }}</div>
    </div>
...
<div class="plate-section">
  <h3>Starters — built for you, run by you</h3>
  <div rv-each="starters">
    <p>
      <button type="button" rv-open-sql="{{ row.accel_script }}" rv-open-sql-title="Accelerate a table">Accelerate a table</button>
      <button type="button" rv-open-sql="{{ row.crawl_script }}" rv-open-sql-title="Build the data catalog">Build the catalog</button>
      <button type="button" rv-open-sql="{{ row.jobs_script }}" rv-open-sql-title="Install maintenance jobs">Install cron jobs</button>
      <button type="button" rv-open="plate:system/health" rv-open-title="System Health">Open System Health &#8594;</button>
    </p>
  </div>
</div>
$tpl$,
  jsonb_build_object(
    'steps', jsonb_build_object('sql', $q$
WITH ut AS (
  SELECT count(*) AS n FROM pg_class c JOIN pg_namespace n2 ON n2.oid = c.relnamespace
  WHERE c.relkind = 'r' AND n2.nspname NOT IN ('pg_catalog', 'information_schema', 'rvbbit', 'cron')
),
accel AS (SELECT count(*) AS n FROM rvbbit.tables),
ops AS (SELECT count(*) AS n FROM rvbbit.operators),
caps AS (SELECT count(*) AS n FROM rvbbit.capability_catalog),
pl AS (SELECT count(*) AS n FROM rvbbit.plates)
SELECT 1 AS ord, 'your tables' AS name, n::text AS value,
       CASE WHEN n = 0 THEN 'warn' ELSE 'ok' END AS tone,
       CASE WHEN n = 0 THEN 'connect or load some data first' ELSE 'user tables in this database' END AS note
FROM ut
UNION ALL
SELECT 2, 'accelerated', n::text,
       CASE WHEN n = 0 THEN 'warn' ELSE 'ok' END,
       CASE WHEN n = 0 THEN 'no columnar copies yet — try the starter below' ELSE 'tables with columnar acceleration' END
FROM accel
UNION ALL
SELECT 3, 'semantic operators', n::text,
       CASE WHEN n = 0 THEN 'warn' ELSE 'ok' END,
       CASE WHEN n = 0 THEN 'install capabilities from the catalog' ELSE 'semantic SQL is ready' END
FROM ops
UNION ALL
SELECT 4, 'capabilities', n::text, 'ok', 'catalog entries — import more from rvbbit.ai'
FROM caps
UNION ALL
SELECT 5, 'plates', n::text, 'ok', 'surfaces installed as rows'
FROM pl
ORDER BY 1
    $q$),
    'starters', jsonb_build_object('sql', $q$
SELECT
  coalesce((SELECT E'-- Accelerate your largest table (edit to taste), then compact it.\n' ||
    'SELECT rvbbit.enable_table(' || quote_literal(rel) || '::regclass, false);' || E'\n' ||
    'SELECT rvbbit.compact(' || quote_literal(rel) || '::regclass);'
    FROM (
      SELECT quote_ident(n.nspname) || '.' || quote_ident(c.relname) AS rel
      FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE c.relkind = 'r'
        AND n.nspname NOT IN ('pg_catalog', 'information_schema', 'rvbbit', 'cron')
        AND NOT EXISTS (SELECT 1 FROM rvbbit.tables t WHERE t.table_oid = c.oid)
      ORDER BY pg_total_relation_size(c.oid) DESC LIMIT 1
    ) pick), '-- connect or load some data first, then re-render this plate') AS accel_script,
  E'-- Crawl the catalog: builds the searchable structure map + fingerprints.\nCALL rvbbit.catalog_crawl_run();' AS crawl_script,
  CASE
    WHEN current_database() = coalesce((SELECT setting FROM pg_settings WHERE name = 'cron.database_name'), 'postgres')
    THEN E'-- This database IS the pg_cron home — install directly.\nSELECT rvbbit.install_maintenance_jobs();'
    ELSE E'-- >>> RUN CONNECTED TO ' || quote_literal(coalesce((SELECT setting FROM pg_settings WHERE name = 'cron.database_name'), 'postgres')) || E' (the pg_cron home db) <<<\nCREATE EXTENSION IF NOT EXISTS pg_cron;\nSELECT cron.schedule_in_database(''rvbbit-maintain'', ''*/15 * * * *'', ''SELECT rvbbit.maintain();'', ' || quote_literal(current_database()) || E');\nSELECT cron.schedule_in_database(''rvbbit-storage-maintain'', ''0 * * * *'', ''SELECT rvbbit.maintain_storage();'', ' || quote_literal(current_database()) || ');'
  ...
  '{}'::jsonb, '[]'::jsonb,
  'rvbbit', 'The product''s own switchboard: readiness cards + built-not-run starters'
);
