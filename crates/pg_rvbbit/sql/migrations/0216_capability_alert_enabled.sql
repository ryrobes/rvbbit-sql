...
--
-- cap_alert nodes carried no enabled/disabled signal even though alerts
-- genuinely have one (rvbbit.enable_alert/disable_alert write
-- rvbbit.alert_control.enabled) — every other kind that has a real
-- configured-vs-not state already exposes it (cap_pack.install_state,
-- cap_brain.enabled, cap_peer_backend.enabled/live). Closes that gap so a
-- capability browser can show "paused" alerts the same way it shows a
-- disabled brain or a paused peer backend, instead of silently treating
-- every alert as active.
--
-- alert_control has no row until an alert is explicitly paused at least
-- once (enable_alert/disable_alert upsert it), so the LEFT JOIN defaults
-- to enabled=true — matching alert_control.enabled's own column default.
--
-- Same precedent as 0215's block 9: one numbered crawl block edited in
-- place, never a change to capability_search() itself.
...
BEGIN
    ...
    BEGIN
        ...
        -- 7) Active alert rules (latest version per name), joined against
        -- alert_control for the real enabled/paused state (enable_alert/
        -- disable_alert write there; no row yet defaults to enabled=true,
        -- matching alert_control.enabled's own column default).
        FOR rec IN SELECT DISTINCT ON (r.name) r.name, r.description,
                          r.condition_spec->>'kind' AS cond_kind, r.cardinality,
                          coalesce(c.enabled, true) AS enabled
                     FROM rvbbit.alert_rules r
                     LEFT JOIN rvbbit.alert_control c ON c.name = r.name
                    ORDER BY r.name, r.version DESC LOOP
            v_doc := rvbbit.capability_doc('cap_alert', rec.name,
                        ...
                        || CASE WHEN NOT rec.enabled THEN ' PAUSED — not currently firing.' ELSE '' END,
                        '', 'cheap', ARRAY['alert','watch']);
            v_node := rvbbit.kg_assert_node('cap_alert', rec.name,
                        jsonb_strip_nulls(jsonb_build_object(
                            'condition_kind', rec.cond_kind, 'enabled', rec.enabled, 'search_doc', v_doc)),
                        1.0, '', 0.0, v_graph);
            v_vec := NULL;
            IF v_embed_ok THEN
                BEGIN v_vec := rvbbit.embed(v_doc, embed_specialist, 'document');
