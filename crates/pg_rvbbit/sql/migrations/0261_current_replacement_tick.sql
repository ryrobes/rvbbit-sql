-- 0261: Make current-only TRUNCATE a first-class full-rebuild state.
--
-- 0260 changes ordinary current-only replacement loads into O(1) baseline
-- invalidations. Re-issue the policy executor so databases that already
-- applied 0258 plan those replacements directly as full rebuilds rather than
-- discovering that requirement through the delta-refresh exception fallback.
...
BEGIN
    ...
    LOOP
        ...
        IF cand.current_replacement_pending THEN
            -- No row tombstones exist for this full replacement, so a delta
            -- can never make the obsolete accelerator baseline readable.
            prop_action := 'full';
            prop_reason := 'current replacement pending';
        ELSIF cand.max_row_groups_before_rebuild IS NOT NULL
