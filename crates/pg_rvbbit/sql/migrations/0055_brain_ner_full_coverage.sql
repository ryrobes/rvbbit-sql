-- 0055_brain_ner_full_coverage — NER over ALL chunks; cap only the expensive LLM triples.
--
-- A long doc (the 158-chunk Compliance SOP) was only enriched for its first p_max_chunks chunks, so a
-- query matching a DEEP chunk got empty per-chunk entities (the "Florida is in the doc but not on the
-- hit" symptom). The triples pass is the costly one (one LLM call per chunk); NER (GLiNER) is cheap, so
-- it should cover the WHOLE document. Decoupled caps:
--   • triples: first p_max_chunks chunks (relations; LLM cost control).
--   • NER:     up to rvbbit.brain_ner_max_chunks (default 400 ≈ whole doc) → comprehensive entity
--              coverage on every chunk, so chunk-scoped entities are populated wherever a hit lands.
...
DECLARE
    ...
    v_ci int := 0;        -- chunk counter (triples only while < p_max_chunks)
    v_ner_cap int;        -- NER chunk ceiling (≈ whole doc)
BEGIN
    ...
    FOR ch IN SELECT chunk_id, text FROM rvbbit.brain_chunks
        ...
        -- (1) Relation triples → only the first p_max_chunks chunks (LLM cost control).
        ...
        -- (2) NER → EVERY chunk (cheap GLiNER specialist) for comprehensive entity coverage.
        IF v_ner_on THEN
