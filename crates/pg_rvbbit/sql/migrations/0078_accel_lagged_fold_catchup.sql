-- Make full accelerator folds write-friendly: build the expensive baseline
-- without a table lock, then take a short handoff lock to export the mutation
-- gap and swap metadata.

CREATE OR REPLACE FUNCTION rvbbit.export_to_parquet_snapshot_visible(rel oid, snapshot text)
RETURNS bigint
LANGUAGE c
STRICT
AS '$libdir/pg_rvbbit', 'export_to_parquet_snapshot_visible_wrapper';

CREATE OR REPLACE FUNCTION rvbbit.export_to_parquet_snapshot_gap(rel oid, snapshot text)
RETURNS bigint
LANGUAGE c
STRICT
AS '$libdir/pg_rvbbit', 'export_to_parquet_snapshot_gap_wrapper';

CREATE OR REPLACE FUNCTION rvbbit.export_to_parquet_snapshot_visible_at(
    rel oid,
    snapshot text,
    first_rg_id bigint,
    generation bigint
...
LANGUAGE c
STRICT
AS '$libdir/pg_rvbbit', 'export_to_parquet_snapshot_visible_at_wrapper';

CREATE OR REPLACE FUNCTION rvbbit.export_to_parquet_snapshot_gap_at(
    rel oid,
    snapshot text,
    first_rg_id bigint,
    generation bigint
...
LANGUAGE c
STRICT
AS '$libdir/pg_rvbbit', 'export_to_parquet_snapshot_gap_at_wrapper';
