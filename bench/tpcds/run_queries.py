"""Run the 99 TPC-DS queries on each selected system."""
...
SYSTEMS = os.environ.get(
    "BENCH_SYSTEMS",
    "rvbbit,duckdb,pg_baseline,citus,hydra,alloydb",
).split(",")
...
def run_one(system: str, sql: str, qid: str) -> tuple[float | None, str]:
    try:
        ...
        if system == "rvbbit_duck_hot":
            ms = run_rvbbit_duck_hot(sql, REPEATS, TIMEOUT_S, label=qid, suite="tpcds")
        ...
        if system == "rvbbit_duck_forced":
            ms = run_rvbbit_duck_hot(sql, REPEATS, TIMEOUT_S, mode="force-duck", label=qid, suite="tpcds")
            return ms, rvbbit_duck_hot_status()
        if system == "rvbbit_duck_hive_forced":
            ms = run_rvbbit_duck_hive_forced(sql, REPEATS, TIMEOUT_S, label=qid, suite="tpcds")
            return ms, rvbbit_duck_hot_status()
        if system == "rvbbit_duck_vortex_forced":
            ms = run_rvbbit_duck_vortex_forced(sql, REPEATS, TIMEOUT_S, label=qid, suite="tpcds")
            return ms, rvbbit_duck_hot_status()
        if system == "rvbbit_datafusion_forced":
            ms = run_rvbbit_datafusion_forced(sql, REPEATS, TIMEOUT_S, label=qid, suite="tpcds")
            return ms, rvbbit_duck_hot_status()
        if system == "rvbbit_datafusion_hive_forced":
            ms = run_rvbbit_datafusion_hive_forced(sql, REPEATS, TIMEOUT_S, label=qid, suite="tpcds")
            return ms, rvbbit_duck_hot_status()
...
def main() -> None:
    ...
    results: dict = {}
    details: dict = {}
    out_path = "/bench/tpcds/results/last_run.json"
