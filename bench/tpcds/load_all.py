"""Load TPC-DS parquet into each benchmark system.
...
Env:
  ...
  RVBBIT_DIRECT_ACCEL_STAGING_MODE
                  source imports generated TPC-DS parquet directly; single_pass
    ...
    try:
        if name == "duckdb":
            ...
            res = load_pg(
                ...
                data_dir,
                post_sql=[f"VACUUM ANALYZE {t}" for t in table_names(data_dir)],
            )
        elif name == "citus":
            from loaders.postgres_loader import load_pg
            res = load_pg(
            ...
            res = load_pg(
                ...
                post_sql=[
                    *[f"VACUUM ANALYZE {t}" for t in table_names(data_dir)],
                    *[f"SELECT google_columnar_engine_add('{t}')" for t in table_names(data_dir)],
                    *[f"SELECT google_columnar_engine_refresh('{t}')" for t in table_names(data_dir)],
                ],
            )
        elif name == "rvbbit":
            from loaders.postgres_loader import load_pg
            ...
            compact_sql = [
                *[f"ANALYZE {t}" for t in table_names(data_dir)],
            ...
            if _hot_load_after_load():
                final_sql.extend(
                    [f"SELECT rvbbit.hot_load('{t}'::regclass)" for t in table_names(data_dir)]
                )
            res = load_pg(
                dsn,
                data_dir,
            ...
            if refresh_mode == "async":
                refresh_sql = "; ".join(
                    [
                        # fresh psql connection: re-pin the tpcds schema so the
                        # unqualified regclass lookups can't hit TPC-H tables
                ...
                pid = _start_async_variant_refresh(
                    dsn,
                    refresh_sql,
                    "/bench/tpcds/results/refresh_layout_variants.log",
                )
...
def _print_direct_accel_loader_summary(results: list[dict]) -> None:
    ...
    if tables:
        ...
        print(f"{'table':<24} {'rows':>12} {'files':>7} {'bytes':>10} {'import(s)':>10}")
        print("-" * 69)
        for item in tables:
...
def main() -> int:
    ...
    missing = [t for t in table_names(data_dir) if not os.path.exists(f"{data_dir}/{t}.parquet")]
    if missing:
        print(f"ERROR: missing TPC-DS parquet in {data_dir}: {', '.join(missing)}")
        return 1
    selected = os.environ.get("BENCH_SYSTEMS", ",".join(DEFAULT_SYSTEMS)).split(",")
