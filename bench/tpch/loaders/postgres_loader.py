...
def _stream_rows(data_dir: str, table: str) -> Iterable[tuple]:
    ...
    stream = con.execute(
        f"SELECT {duckdb_select_list(table)} FROM read_parquet({_sql_literal(f'{data_dir}/{table}.parquet')})"
    )
    while True:
...
def _write_direct_accel_chunks(data_dir: str, table: str) -> tuple[list[str], float, Path | None]:
    ...
    run_dir = DIRECT_ACCEL_DIR / f"tpch-{table}-{os.getpid()}-{int(time.time() * 1000)}"
    if run_dir.exists():
        shutil.rmtree(run_dir)
    run_dir.mkdir(parents=True, exist_ok=True)

    source_sql = f"SELECT {duckdb_select_list(table)} FROM read_parquet({_sql_literal(data_path)})"
...
def _measure_size(cur, using: str | None) -> dict:
    names = table_names()
    if using == "rvbbit":
        cur.execute(
            """
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        with conn.cursor() as cur:
            ...
            for table in table_names():
                cur.execute(ddl_postgres(table, using=using))
            ...
            size_info = _measure_size(cur, using)
            result = {
                "rows": rows,
